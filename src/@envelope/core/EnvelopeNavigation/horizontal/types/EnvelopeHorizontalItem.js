...
import {styled, ListItemText, ListItem} from "@mui/material";
...
function EnvelopeNavHorizontalItem(props) {
    const { item } = props;
    ...
    return useMemo(
        () => (
            <StyledListItem
                ...
                className={classNames('fuse-list-item', item.active && 'active')}
                end={item.end}
                role="button"
                sx={item.sx}
...
EnvelopeNavHorizontalItem.propTypes = {
...
EnvelopeNavHorizontalItem.defaultProps = {};

const NavHorizontalItem = withRouter(memo(EnvelopeNavHorizontalItem));
