...
function EnvelopeNavVerticalTab(props) {
          ...
          {dense ? (
            <Tooltip title={item.title || ''} placement="right">
              <div className="w-32 h-32 min-h-32 flex items-center justify-center relative">
                {item.icon ? (
                  <EnvelopeSvgIcon
                    className={classNames('fuse-list-item-icon', item.iconClass)}
                    color="action"
                  >
                    {item.icon}
                  </EnvelopeSvgIcon>
...
EnvelopeNavVerticalTab.propTypes = {
...
EnvelopeNavVerticalTab.defaultProps = {};

const NavVerticalTab = EnvelopeNavVerticalTab;
