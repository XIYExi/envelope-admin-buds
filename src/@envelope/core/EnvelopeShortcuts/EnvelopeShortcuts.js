...
function EnvelopeShortcuts(props) {
            ...
            <Link to={item.url || ''} role='button'>
                <MenuItem key={item.id}>
                    <ListItemIcon className="min-w-40">
                        {item.icon ? (
                            <EnvelopeSvgIcon>{item.icon}</EnvelopeSvgIcon>
                        ) : (
    ...
    return(
        <div
            className={classNames('flex flex-1', props.className, props.variant ==='vertical' && 'flex-col grow-0 shrink')}
        >
            {
                useMemo(() =>{
                    const container = {
                    ...
                    return(
                        ...
                        >
                                ...
                                shortcutItems.map((_item) =>
                                    _item && (
                                        <Link to={_item.url} key={_item.id} role='button'>
                                            <Tooltip
                                                title={_item.title}
                                                placeholder={props.variant === 'horizontal' ? 'bottom' : 'left'}
                                            >
                                                <IconButton
                                                ...
                                                >
                                                    {
                                                        _item.icon ? (
                                                            <EnvelopeSvgIcon>{_item.icon}</EnvelopeSvgIcon>
                                                        ):(
                            ...
                            <Tooltip title='Click to add/remove shortcuts'
                            ...
                            >
                                <IconButton
                                    ...
                                    size="large"
                                >
                                    <EnvelopeSvgIcon sx={{ color: amber[600] }}>heroicons-solid:star</EnvelopeSvgIcon>
                                </IconButton>
                            </Tooltip>
                        </motion.div>
                    );
...
EnvelopeShortcuts.propTypes = {};
EnvelopeShortcuts.defaultProps = {
    variant: 'horizontal',
}

export default memo(EnvelopeShortcuts);